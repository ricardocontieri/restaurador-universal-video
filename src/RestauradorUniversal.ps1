...
function Get-ArchiveMetadataArgs {
    ...
    $argsOut += @(
        "-metadata",
        "metadata_schema=RVC_ARCHIVE_DATE_V1",
        "-metadata",
        (
...
function Get-MetadataTagValueCI {
    param(
        [object]$Tags,
        [string]$Name
    )
...
function Test-ArchiveMetadata {
    ...
    if (
        ...
        return [pscustomobject]@{
            ...
            CoreDateOk = $false
            CustomTagsOk = $false
            Status = "FFPROBE_FALHOU"
            CreationTimeFound = ""
            Warning = "FFprobe não conseguiu ler metadados do master."
        }
    }
    ...
    try {
        ...
        if (
            ...
            $creationCandidates +=
                [pscustomobject]@{
                    Source = "format.creation_time"
                    Value = $globalCreation
                }
        }

        $streamIndex = 0

        foreach ($stream in @($mo.streams)) {
            $streamCreation =
                Get-MetadataTagValueCI `
                    -Tags $stream.tags `
                    -Name "creation_time"
            ...
            if (
                ...
                $creationCandidates +=
                    [pscustomobject]@{
                        Source =
                            "stream{0}:{1}.creation_time" -f `
                                $streamIndex,
                                ([string]$stream.codec_type)
                        Value = $streamCreation
                    }
            }
        ...
        if (
            ...
            foreach ($candidate in $creationCandidates) {
                if (
                    [string]$candidate.Value -like
                    "$($DateInfo.CanonicalDate)*"
                ) {
                    $coreDateOk = $true
                    $creationFound =
        ...
        # Tags de proveniência são desejáveis, mas não podem derrubar
        # um master cujo creation_time canônico foi corretamente gravado.
        # A proveniência completa também existe em METADADOS_DATAS.csv.
        $restorationTime =
            Get-MetadataTagValueCI `
                -Tags $formatTags `
        ...
        $originalCapture =
            Get-MetadataTagValueCI `
                -Tags $formatTags `
                -Name "original_capture_time"

        $originalSource =
            Get-MetadataTagValueCI `
                -Tags $formatTags `
                -Name "original_date_source"

        $originalConfidence =
            Get-MetadataTagValueCI `
                -Tags $formatTags `
                -Name "original_date_confidence"
        ...
        if (
            ...
            $customTagsOk = $false
            $customWarnings += "restoration_time ausente"
        }
        ...
        if (
            ...
            if (
                ...
                $customTagsOk = $false
                $customWarnings += "original_capture_time ausente"
            }
            elseif (
                $originalCapture -notlike
                "$($DateInfo.CanonicalDate)*"
            ) {
                $customTagsOk = $false
                $customWarnings +=
                    "original_capture_time não coincide com a data canônica"
            }
            ...
            if (
                ...
                $customTagsOk = $false
                $customWarnings += "original_date_source ausente"
            }
            ...
            if (
                ...
                $customTagsOk = $false
                $customWarnings += "original_date_confidence ausente"
            }
        }
        ...
        if (-not $coreDateOk) {
            return [pscustomobject]@{
                Ok = $false
                CoreDateOk = $false
                CustomTagsOk = $customTagsOk
                Status = "CREATION_TIME_CANONICO_AUSENTE"
                CreationTimeFound =
                    @(
                        $creationCandidates |
                        ForEach-Object {
                ...
                Warning =
                    (
                        "Nenhum creation_time coincide com {0}." -f `
                            $DateInfo.CanonicalDate
                    )
            }
        }
        ...
        return [pscustomobject]@{
            ...
            Status =
                if ($customTagsOk) {
                    ...
                    "OK_DATA_CORE_SIDECAR_PROVENIENCIA"
                }
            CreationTimeFound = $creationFound
            Warning = $warning
        }
    }
    catch {
...
function Rewrite-ExistingFinalMetadata {
    ...
    $metaValidation =
        Test-ArchiveMetadata `
            -File $temp `
            -DateInfo $DateInfo `
            -DiagnosticFile $metaProbe

    if (-not $metaValidation.Ok) {
        throw (
            "Validação da data arquivística principal falhou: {0}. {1}. Diagnóstico: {2}" -f `
                $metaValidation.Status,
                $metaValidation.Warning,
                $metaProbe
        )
    }
...
function Mux-Final {
    ...
    $metadataProbe =
        Join-Path `
            $WorkDir `
            "07_METADATA_FFPROBE.json"

    $metadataValidation =
        Test-ArchiveMetadata `
            -File $FinalFile `
            -DateInfo $DateInfo `
            -DiagnosticFile $metadataProbe

    if (-not $metadataValidation.Ok) {
        throw (
            "Data arquivística principal não passou na validação pós-mux: {0}. {1}. Diagnóstico: {2}" -f `
                $metadataValidation.Status,
                $metadataValidation.Warning,
                $metadataProbe
        )
    }

    if (-not $metadataValidation.CustomTagsOk) {
        Write-Host (
            "       AVISO metadata auxiliar: {0}. A data canônica no MP4 está válida; proveniência completa permanece no CSV." -f `
                $metadataValidation.Warning
        ) -ForegroundColor DarkYellow
    }
    ...
    if (
        -not [string]::IsNullOrWhiteSpace(
            [string]$metadataValidation.CreationTimeFound
        )
    ) {
        Write-Host (
            "       Metadata validada: {0}" -f `
                $metadataValidation.CreationTimeFound
        ) -ForegroundColor Green
    }
...
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host " RESTAURADOR UNIVERSAL DE VIDEOS 1.0.0 PUBLIC - SMART DUAL GPU / ARCHIVE DATE VALIDATION / 1080P / STAB / PILLAR BLUR" `
    -ForegroundColor Cyan
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host ""
    ...
    foreach ($item in $inventory) {
        ...
        try {
            ...
            if ($mr.Status -eq "OK") {
                ...
                $canCheckpointMetadata = $false

                if (
                    (Test-Path -LiteralPath $exactFrameFile) -and
                    (Test-Path -LiteralPath $finalFile)
                ) {
                    $expectedExact = 0L
                    ...
                    if (
                        ...
                        if (
                            $finalInfoForMeta -and
                            $finalInfoForMeta.Width -eq 1920 -and
                            $finalInfoForMeta.Height -eq 1080 -and
                            [long]$finalInfoForMeta.FrameCount -eq
                            [long]$expectedExact
                        ) {
                            $canCheckpointMetadata = $true
                        }
                    }
                }

                if ($canCheckpointMetadata) {
                    New-Item `
                        -ItemType Directory `
                        -Path $itemWorkDir `
                        -Force |
                    ...
                    New-Item `
                        -ItemType File `
                        -Path $metadataDoneFile `
                        -Force |
                        Out-Null

                    Write-Host (
                        "       Checkpoint metadata criado; frame count final confere com o censo exato."
                    ) -ForegroundColor DarkGreen
                }
                else {
                    Write-Host (
                        "       Metadata atualizada, mas sem checkpoint de skip: não foi possível confirmar o frame count contra 00_FRAMECOUNT_EXATO.txt."
                    ) -ForegroundColor DarkYellow
                }
...
foreach ($item in $inventory) {
    ...
    $exactFrameCheckpoint =
        Join-Path `
            $workDir `
            "00_FRAMECOUNT_EXATO.txt"

    $canFastResumeFinal = $false
    $fastResumeInfo = $null

    if (
        (-not $Forcar) -and
        (Test-Path -LiteralPath $finalFile) -and
        (Test-Path -LiteralPath $finalMetadataDone) -and
        (Test-Path -LiteralPath $exactFrameCheckpoint)
    ) {
        $expectedFinalFrames = 0L

        $expectedText =
            (
                Get-Content `
                    -LiteralPath $exactFrameCheckpoint `
                    -Raw `
                    -ErrorAction SilentlyContinue
            ).Trim()
        ...
        if (
            ...
            if (
                $fastResumeInfo -and
                $fastResumeInfo.Width -eq 1920 -and
                $fastResumeInfo.Height -eq 1080 -and
                [long]$fastResumeInfo.FrameCount -eq
                [long]$expectedFinalFrames
            ) {
                $fastMetaProbe =
                    Join-Path `
                        $workDir `
                        "07_METADATA_FAST_RESUME_FFPROBE.json"

                $fastMetaValidation =
                    Test-ArchiveMetadata `
                        -File $finalFile `
                        -DateInfo $item.DateInfo `
                        -DiagnosticFile $fastMetaProbe

                if ($fastMetaValidation.Ok) {
                    $canFastResumeFinal = $true
                }
            }
        }
    }

    if ($canFastResumeFinal) {
        Write-Host (
            "  [RESUME FINAL] master 1920x1080 + {0} frames + metadata arquivística válidos; nada a reprocessar." -f `
                $fastResumeInfo.FrameCount
        ) -ForegroundColor Green
