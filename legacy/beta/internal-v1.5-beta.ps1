...
function Invoke-CudaDecodeSelfTest {
    param(
        [string]$File,
        [string]$Log,
        [int]$TestW,
...
function Invoke-QsvDecodeSelfTest {
    param(
        [string]$File,
        [object]$Info,
        [string]$Log,
    ...
    $decoder = Get-QsvDecoderName $Info.VideoCodec

    if (
        $SemIntelQSV -or
        -not $script:QsvBuildAvailable -or
        -not (Test-QsvDecoderPresent $decoder)
    ) {
        $reason =
            if ($SemIntelQSV) {
...
function Test-GpuDecodeForFile {
    ...
    $cudaLog =
        Join-Path `
            $WorkDir `
            "00_GPU_DECODE_NVIDIA_SELFTEST.log"

    $qsvLog =
        Join-Path `
            $WorkDir `
            "00_GPU_DECODE_INTEL_QSV_SELFTEST.log"

    Write-Host (
        "       Testando NVIDIA e Intel para decode de {0} ({1}x{2}, {3} frames)..." -f `
            $Info.VideoCodec,
            $Info.Width,
            $Info.Height,
            $testFrames
    ...
    if ($cuda.Ok) {
        Write-Host (
            "       NVIDIA NVDEC/CUDA: OK ({0:N0} ms)." -f `
                $cuda.ElapsedMs
        ) -ForegroundColor Green
    }
    else {
        Write-Host (
            "       NVIDIA NVDEC/CUDA: indisponível (exit {0})." -f `
                $cuda.ExitCode
        ) -ForegroundColor Yellow
    }
    ...
    elseif ($cuda.Ok -and $qsv.Ok) {
        # QSV precisa ganhar com margem para substituir uma rota CUDA madura.
        if ($qsv.ElapsedMs -lt ($cuda.ElapsedMs * 0.90)) {
            $chosen = $qsv
        }
        else {
            $chosen = $cuda
    ...
    if ($null -ne $chosen) {
        ...
        if ($chosen.Backend -eq "QSV") {
            $script:UseQsvDecode = $true
            $script:DecodeBackend = "QSV"
            $script:QsvDecoder = $chosen.Decoder
            ...
            Write-Host (
                "       ROTA ESCOLHIDA: Intel QSV ({0}) -> filtros CPU -> {1}." -f `
                    $script:QsvDecoder,
                    $encoderName
            ) -ForegroundColor Green
        }
            ...
            Write-Host (
                "       ROTA ESCOLHIDA: NVIDIA NVDEC/CUDA -> filtros CPU -> {0}." -f `
                    $encoderName
            ) -ForegroundColor Green
        }
    ...
    Write-Host (
        "       ROTA ESCOLHIDA: CPU decode -> filtros CPU -> {0}." -f `
            $encoderName
    ) -ForegroundColor Yellow

    $cudaReason = Get-LastLogReason $cudaLog
    $qsvReason = Get-LastLogReason $qsvLog

    if (-not [string]::IsNullOrWhiteSpace($cudaReason)) {
        Write-Host (
            "       NVIDIA: {0}" -f `
                $cudaReason
        ) -ForegroundColor DarkYellow
    ...
    if (-not [string]::IsNullOrWhiteSpace($qsvReason)) {
        Write-Host (
            "       Intel QSV: {0}" -f `
                $qsvReason
        ) -ForegroundColor DarkYellow
...
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host " RESTAURADOR UNIVERSAL DE VIDEOS V1.5 - DUAL GPU / 1080P / STAB / PILLAR BLUR" `
    -ForegroundColor Cyan
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host ""
...
if ($PreferirIntelQSV) {
    ...
    Write-Host `
        "Política Auto: NVIDIA e Intel são testadas; QSV entra se for claramente mais rápida ou se CUDA falhar." `
        -ForegroundColor Cyan
}
