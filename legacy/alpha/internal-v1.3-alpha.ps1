...
function Test-GpuDecodeForFile {
    ...
    # Encode e decode são capacidades independentes.
    # Mesmo com NVENC disponível, codecs antigos (ex.: vários AVI)
    # podem não ter decoder NVDEC/CUDA correspondente.
    $script:UseGpuDecode = $false

    if (-not $script:UseGpuEncode) {
        Write-Host (
            "       Decode: CPU (NVENC indisponível; pipeline inteiro em CPU)."
        ) -ForegroundColor Yellow
    ...
    Write-Host (
        "       Testando decode CUDA deste arquivo ({0}, {1}x{2})..." -f `
            $Info.VideoCodec,
            $Info.Width,
            $Info.Height
    ...
    if ($exit -eq 0) {
        ...
        Write-Host (
            "       Decode: NVDEC/CUDA OK | Encode: NVENC."
        ) -ForegroundColor Green
    ...
    Write-Host (
        "       Decode CUDA não é compatível com este arquivo (exit {0})." -f `
            $exit
    ) -ForegroundColor Yellow

    Write-Host (
        "       Fallback automático: CPU decode -> filtros -> NVENC."
    ) -ForegroundColor Yellow
...
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host " RESTAURADOR UNIVERSAL DE VIDEOS V1.3 - 1080P / STAB / PILLAR BLUR" `
    -ForegroundColor Cyan
Write-Host "============================================================" `
    -ForegroundColor Cyan
Write-Host ""
...
if ($script:UseGpuEncode) {
    Write-Host `
        "GPU: NVIDIA NVENC disponível. Encode usará GPU; decode será testado individualmente em cada vídeo." `
        -ForegroundColor Green
}
else {
    Write-Host `
        "GPU NVENC não disponível. Encode e decode seguirão em CPU/libx264." `
        -ForegroundColor Yellow
}
